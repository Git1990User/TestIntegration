...
public class LoginScenarios extends ExtentReportBaseClass  {
	...
	AssertJUnit.assertEquals(driver.findElementByXPath("//*[@id=\"root\"]/main/div/div/div/div[2]/div/label[3]").getText(), "Invalid user credentials");
	driver.close();
	...
	AssertJUnit.assertEquals(driver.findElementById("email-helper-text").getText(), "Incorrect Username");	
	AssertJUnit.assertEquals(driver.findElementById("password-helper-text").getText(), "Password is required and must not be less than 4 characters");	
	driver.close();
